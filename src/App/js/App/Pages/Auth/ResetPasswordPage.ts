...
class AuthResetPasswordPage extends FormPage {
    ...
    protected async onActivate(args?: any): Promise<void> {
        ...
        try {
            await this.api.resetTokenValidate({key: this.model.key, token: this.model.token});
    ...
    public async save(viewModel: validate.ValidateableViewModel) {
        try {
            await this.api.resetPassword({key: this.model.key, token: this.model.token, newPassword: this.model.newPassword(), newPasswordConfirm: this.model.newPasswordConfirm()});
...
export default {
    ...
    routingTable: [
        { name: 'auth.resetPassword', path: '/reset-password' }
    ],
